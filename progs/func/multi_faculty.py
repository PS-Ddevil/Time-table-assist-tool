...
master2.geometry("750x550+300+300")
...
ws = wb.active
w = Label(frame2, text="Course",borderwidth=2,font='Helvetica 20 bold')
w.grid(row=0,column=0,columnspan=1,sticky=W+E+N+S)
w.config(width=20)
w = Label(frame2, text="Multiple Faculty",borderwidth=2,font='Helvetica 20 bold')
w.grid(row=0,column=1,columnspan=2,sticky=W+E+N+S)
w.config(width=20)
for i in range(2,ws.max_row+1):
    w = Label(frame2, text=ws.cell(i,1).value,highlightbackground="black", highlightcolor="black", highlightthickness=1, anchor="w")
    w.grid(row=i-1,column=0,columnspan=1,sticky=W+E+N+S)
    w.config(width=30)
    w = Label(frame2, text=ws.cell(i,2).value,highlightbackground="black", highlightcolor="black", highlightthickness=1, anchor="w")
...
w.pack()
w = Label(frame, fg="BLUE",text="'/Time-table-assist-tool/src/tmp/baskets/course_faculty_optional.xlsx'")
w.pack()
w = Label(frame, fg="RED",text="and do the following changes to it.")
w.pack()
